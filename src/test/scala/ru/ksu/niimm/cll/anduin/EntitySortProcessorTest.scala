...
import com.twitter.scalding.{TupleConversions, JobTest}
...
object EntitySortProcessorTestSpec extends Specification with TupleConversions {
  "Entity processor " should {
    JobTest("ru.ksu.niimm.cll.anduin.EntitySortProcessor").
      ...
      // 1st row
      ("0", "1\t<http://example.org/Dvaleriafraschetti>\t\"valeria\tfraschetti\""),
      // 2nd row
      ("1", "0\t<http://example.org/Dvaleriafraschetti>\t\"valeria fraschetti\""),
      // 3rd row
      ("2", "0\t<http://example.org/Dvaleriafraschetti>\t\"Female\""),
      // 4th row
      ("3", "0\t<http://example.org/Dvaleriafraschettz>\t\"Female\"")
    )).
